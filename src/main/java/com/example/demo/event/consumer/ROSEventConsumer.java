...
public class ROSEventConsumer {
    ...
    private void optimizeRoute(WarehouseConfirmedEvent event) {
        try {
            ...
            Route route = saveOptimizedRoute(event, rosResponse);
     ...
     */
    private Route saveOptimizedRoute(WarehouseConfirmedEvent event, RouteOptimizeResponse rosResponse) {
        Route route = new Route();
        route.setRouteDate(LocalDate.now());
        route.setStatus("OPTIMIZED");
        route.setEstimatedTime(rosResponse.getEstimatedDuration() != null ? rosResponse.getEstimatedDuration() : 0);
